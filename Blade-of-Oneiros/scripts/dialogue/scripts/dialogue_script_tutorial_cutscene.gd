...
func run(orchestrator: DialogueOrchestrator) -> void:
	var swordsman_node := orchestrator._get_character_node("swordsman")
	var pos :Vector2 = swordsman_node.global_position
	...
	orchestrator.action("swordsman", "move", Vector2(pos.x + 12,pos.y), 110.0, "right")
	orchestrator.action("swordsman", "move", Vector2(pos.x,pos.y), 110.0, "down")
	var chorus_voices := [
		{
	...
	orchestrator.npc_moveto(Vector2(pos.x + 18,pos.y - 10))
	orchestrator.npc_face("right")
	orchestrator.npc_appear(0.8)
