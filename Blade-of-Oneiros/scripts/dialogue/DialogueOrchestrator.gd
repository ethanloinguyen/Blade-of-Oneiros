...
var _script_paths: Dictionary = {
	...
	"tooltip_lever": "res://scripts/dialogue/scripts/tooltips/dialogue_script_tooltip_lever.gd",
	"tooltip_fireplace": "res://scripts/dialogue/scripts/tooltips/dialogue_script_tooltip_fireplace.gd",
	"tooltip_block": "res://scripts/dialogue/scripts/tooltips/dialogue_script_tooltip_block.gd",
	"tooltip_armor": "res://scripts/dialogue/scripts/tooltips/dialogue_script_tooltip_armor.gd",
}
...
func on_dialogue_finished() -> void:
	var finished_id := _current_script_id
...
func npc_disappear(character_id: String, duration: float = 0.5) -> void:
...
func _play_next_step() -> void:
	...
	match step["kind"]:
		...
		"wait":
			_run_wait_step(step)
		"parallel_action":
			_run_parallel_action_step(step)
		_:
			_play_next_step()
...
func _show_line_step(step: Dictionary) -> void:
	...
	print("DEBUG: _show_line_step text='", text, "' overrides=", overrides)
...
func npc_moveto(character_id: String, dest: Vector2, facing: String = "", speed: float = 140.0) -> void:
	action(character_id, "move", dest, speed, facing)
...
func _on_parallel_action_one_finished() -> void:
	_parallel_actions_remaining -= 1
	if _parallel_actions_remaining <= 0:
...
func _run_parallel_action_step(step: Dictionary) -> void:
	...
	_parallel_actions_remaining = actions.size()

	for action_step in actions:
		var character_id: String = action_step.get("character_id", "")
		var action_type: String = action_step.get("action_type", "")
		var arg1 = action_step.get("arg1", null)
		var arg2 = action_step.get("arg2", null)
		var arg3 = action_step.get("arg3", null)

		var target := _get_character_node(character_id)
		if target == null:
			push_warning("DialogueOrchestrator: Could not resolve character_id '%s' in parallel_action" % character_id)
		...
		match action_type:
			"move":
				...
				# Already at or basically at the destination
				if distance < 1.0:
					_on_parallel_action_one_finished()
					continue
				...
				if typeof(arg3) == TYPE_STRING:
					facing_override = arg3

				var dir_str := facing_override
				if dir_str == "":
					var delta := dest - current_pos
				...
				tween.finished.connect(_on_parallel_action_one_finished)
